...
pub fn collide_bodies(mut bodies: Vec<Body>) -> Vec<Body> {
    ...
    for i in 0..bodies.len() {
        ...
        for j in i..last {
            ...
            bodies[i].current_position = bodies[i].current_position - normal * mass_ratio_2 * delta;
            bodies[j].current_position = bodies[j].current_position + normal * mass_ratio_1 * delta;
        }
    }
    bodies
}
...
#[cfg_attr(feature = "wasm", wasm_bindgen)]
pub fn constraint_rectangle(circle: Body, top_left: Vector2, bottom_right: Vector2) -> Body {
